...
A Streamlit dashboard sits on top of the same pipeline for a visual walkthrough of the results — cointegration scan, naive-vs-walk-forward metric cards, an interactive equity curve, and the per-window breakdown. Every number is loaded from `results.json`, which is generated by `scripts/precompute_results.py` running the real code in `src/`; nothing here is mocked for the UI.

![Dashboard walkthrough](docs/demo_ui.gif)

```bash
python scripts/precompute_results.py   # writes results.json from the real pipeline
streamlit run app.py                    # launches the dashboard at localhost:8501
```

A terminal-only recording of the raw CLI pipeline is also available for reference:
